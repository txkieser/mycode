...
## Define our new funtion called bootstrapper and the expected arguments (all five)
def bootstrapper(dev_type, dev_ip, dev_un, dev_pw, config):
    try:
        ...
        open_connection.send_command_expect('write menory') # write the memory 
        open_connection.disconnect() # close the open connection

        return True # Everything worked! - "Return TRUE when complete
    except:
        return False # Something failed during the configuration proccess - "Return FALSE if fails"
